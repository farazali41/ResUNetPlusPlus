...
if __name__ == "__main__":
    ## Path
    file_path = "files/"
    model_path1 = "files/resunetplusplus1.h5"
    model_path2 = "files/resunetplusplus2.h5"
    model_path3 = "files/resunetplusplus3.h5"
    ...
    train_gen1 = iterSample(iterable = train_gen, samplesize = 16000)
    train_gen2 = iterSample(iterable = train_gen, samplesize = 16000)
    train_gen3 = iterSample(iterable = train_gen, samplesize = 16000)
    ...
    model1.compile(loss=dice_loss, optimizer=optimizer, metrics=metrics)

    csv_logger = CSVLogger(f"{file_path}resunet1_{batch_size}.csv", append=False)
    checkpoint = ModelCheckpoint(model_path1, verbose=1, save_best_only=True)
    ...
    optimizer = Nadam(lr)
    metrics = [Recall(), Precision(), dice_coef, MeanIoU(num_classes=2)]
    model2.compile(loss=dice_loss, optimizer=optimizer, metrics=metrics)

    csv_logger = CSVLogger(f"{file_path}resunet2_{batch_size}.csv", append=False)
    checkpoint = ModelCheckpoint(model_path2, verbose=1, save_best_only=True)
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=5, min_lr=1e-6, verbose=1)
    early_stopping = EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=False)
    callbacks = [csv_logger, checkpoint, reduce_lr, early_stopping]
    ...
    arch = ResUnetPlusPlus(input_size=image_size)
    model3 = arch.build_model()

    optimizer = Nadam(lr)
    metrics = [Recall(), Precision(), dice_coef, MeanIoU(num_classes=2)]
    model3.compile(loss=dice_loss, optimizer=optimizer, metrics=metrics)

    csv_logger = CSVLogger(f"{file_path}resunet3_{batch_size}.csv", append=False)
    checkpoint = ModelCheckpoint(model_path3, verbose=1, save_best_only=True)
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=5, min_lr=1e-6, verbose=1)
    early_stopping = EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=False)
    callbacks = [csv_logger, checkpoint, reduce_lr, early_stopping]

    model3.fit_generator(train_gen,
            validation_data=valid_gen,
            steps_per_epoch=train_steps,
            validation_steps=valid_steps,
            epochs=epochs,
